(function() {
  ...
  module.exports = function(BPM) {
    ...
    BPM.on("install", function(next) {
      ...
      this.progress.emit("fa-copy-parts");
      return fs.copy(path.join(this.runtime.fa_lib, "font"), path.join(this.dir, "font"), function(err) {
        if (err) {
          return next(err);
        }
        return fs.copy(path.join(_this.runtime.fa_lib, "less"), path.join(_this.dir, "less"), function(err) {
          if (err) {
            return next(err);
          }
          return async.parallel([
            function(cb) {
              ...
              return utils.fix_file(path.join(_this.dir, "less", "font-awesome.less"), /@FontAwesomePath(.*?);/i, "@FontAwesomePath: \"" + options.path + "\";", cb);
            }, function(cb) {
              return utils.fix_file(path.join(_this.dir, "less", "bootstrap.less"), "sprites.less", "font-awesome.less", cb);
            }
          ], next);
        });
      });
