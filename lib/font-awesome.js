...
module.exports = function(BPM) {
  ...
  BPM.on("download", function(next) {
    ...
    return fs.copy(path.join(this.runtime.fa_lib, "build/assets/font-awesome/less"), to_folder, function(err) {
      if (err) {
        return next(err);
      }
  ...
  BPM.on("install", function(next) {
    ...
    return fs.copy(path.join(this.runtime.fa_lib, "build/assets/font-awesome/font"), path.join(this.dir, "font"), next);
  });
